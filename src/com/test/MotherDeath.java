...
/**
 * Servlet implementation class MotherDeath
 */
@WebServlet("/MotherDeath")
public class MotherDeath extends HttpServlet {
...
try {
			...
			String fileName = "C:\\Users\\chronin\\Desktop\\mhis-report\\MothersDeath_report.pdf";
			...
			Paragraph para1 = new Paragraph("MATERNITY REPORT",f);
			document.add(para1);
			...
			String query =  "select mother,cardNoDate,deliveryDate from landd where reportName='Mother Death'";
			...
			c1 = new PdfPCell(new Phrase("CARD NUMBER DATE",f1));
			c1.setBackgroundColor(GrayColor.LIGHT_GRAY);
			c1.setHorizontalAlignment(Element.ALIGN_CENTER);
			table.addCell(c1);
			...
			while(rs.next()) {
				PdfPCell c2 = new PdfPCell(new Phrase(rs.getString("mother") ));
				table.addCell(c2);
				c2 = new PdfPCell(new Phrase(rs.getString("cardNoDate") ));
				table.addCell(c2);
